<template>
  <el-dialog :title="typeName + ' - 历史欠交学生'" :visible.sync="dialogVisible" width="80%" class="unfinished-students-dialog">
    <div style="margin-bottom: 10px;">
      ...
      <el-button
          @click="$emit('sort', 'studentNumber', 'asc')"
          :type="sortField === 'studentNumber' && sortOrder === 'asc' ? 'primary' : ''">
        学号升序
      </el-button>
      <el-button
          @click="$emit('sort', 'studentNumber', 'desc')"
          :type="sortField === 'studentNumber' && sortOrder === 'desc' ? 'primary' : ''">
        学号降序
      </el-button>
    </div>
    <el-table :data="students" style="width: 100%" :max-height="400" :row-class-name="tableRowClassName">
  ...
  methods: {
    tableRowClassName({row, rowIndex}) {
      ...
      if (this.sortField === 'date') {
        groupKey = row.date
      } else if (this.sortField === 'studentNumber') {
      ...
      if (groupKey !== undefined) {
        ...
        for (let i = 0; i < this.students.length; i++) {
          let key
          if (this.sortField === 'date') {
            key = this.students[i].date
          } else if (this.sortField === 'studentNumber') {
            key = this.students[i].studentNumber
          }
...
/* 历史欠交学生弹窗中的标记完成按钮特殊样式 */
.unfinished-students-dialog .el-button--danger:not(:disabled):hover {
  background: linear-gradient(90deg, #ff6b6b, #ff7b7b) !important;
  border: none !important;
  color: white !important;
}
